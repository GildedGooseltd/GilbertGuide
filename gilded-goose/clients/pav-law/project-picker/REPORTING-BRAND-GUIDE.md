...
**Standardized tile chrome:** Monthly KPIs tiles all use **Revenue tile chrome** via `.kpi-goal-card`: royal-dim → paper gradient, 2px `var(--gg-royal-border)`, 12px radius, shared padding, track table under the gauge. Do not emit plain paper `.kpi-stat-card` inside `.kpi-goals-grid`. CSS also forces any leftover `.kpi-stat-card` in that grid onto the same chrome. Classes such as `.kpi-stat-attention` may remain as data hooks, but must not add yellow, red, green, or blue borders, left rules, backgrounds, or washes. Use written status, value color, and the single corner check or ✕.
