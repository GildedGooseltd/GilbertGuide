# Yelp Ads


| Field                  | Value                                                                 |
| ---------------------- | --------------------------------------------------------------------- |
| Priority               | 12                                                                    |
| Fee                    | 900                                                                   |
| Category               | Paid Media                                                            |
| Campaign type          | Yelp Ads                                                              |
| Status                 | available                                                             |
| Publish status         | unpublished                                                           |
| Estimated leads gained | 4–10/mo once promo + upgrades + tracking are live                     |
| Keywords               | yelp, ads, promo, upgrade, hubspot, utm, tracking, phone, criminal, dv, assault, felony, military, colorado springs |
| Parent                 | DigProf                                                               |
| Reference link         | https://pav.law/                                                      |


---



## Summary

- Yelp Ads and listing performance for Pav Law criminal defense — not a military-only campaign
- Boost winner lanes from Ads evidence: military · domestic violence · assault · felonies; keep auto/DUI off the Yelp lead story
- Spend the $300 Yelp ad credit / promo while the listing is live; keep daily budget tight and measured
- Test Yelp platform upgrades one at a time — keep only what improves leads and calls
- Add HubSpot-tracked links on the listing and posts so GA4/HubSpot can attribute Yelp activity
- Switch the Yelp phone path to the HubSpot number only after the team is trained and using the HubSpot app for calls — gated on HsVoip / LsaCall



## Value icons


| Show | Name       |
| ---- | ---------- |
| [ ]  | Foundation |
| [ ]  | Retainer   |
| [x]  | Leads      |
| [x]  | CRM        |
| [x]  | HubSpot    |
| [ ]  | SEO        |
| [x]  | Referrals  |
| [x]  | Analytics  |
| [x]  | Finance    |
| [x]  | Intake     |
| [x]  | Creative   |



## KPI links


| Show | Name                            |
| ---- | ------------------------------- |
| [ ]  | Total leads                     |
| [ ]  | New cases                       |
| [ ]  | Key Channel Activity            |
| [ ]  | Pipeline / CRM completeness     |
| [ ]  | Spend Waste                     |
| [x]  | Campaign cost efficiency        |
| [ ]  | Intake conversion               |
| [x]  | Lead channel mix                |
| [x]  | Organic / local search presence |
| [ ]  | Avg. Cost per Call              |
| [x]  | Creative / channel response     |
| [x]  | Cost per lead                   |
| [x]  | Reviews by channel              |
| [ ]  | Referral Network                |
| [ ]  | Website / SEO contribution      |
| [ ]  | Missed Opportunity              |
| [ ]  | CRM follow-up discipline        |
| [x]  | Answered Calls                  |
| [x]  | Speed to lead                   |
| [ ]  | Intake coverage / after-hours   |
| [ ]  | Ops backlog / open tasks        |
| [ ]  | Avg case fee                    |



## Information needed

- Confirm remaining $300 promo / credit balance and end date
- List which Yelp upgrades are currently on vs available to test
- Confirm HubSpot tracking URL set for website + CTA buttons
- Confirm Casey / Romina HubSpot app call readiness before any phone swap



## WIP

- Apply $300 Yelp promo / credit against live ads; log spend vs leads weekly
- Rotate platform upgrades one feature at a time; keep winners, drop duds
- Wire HubSpot/UTM links on listing website, Connect posts, and ad destinations
- Custom ad text + keyword boosts for criminal lanes — military as one boost, not the whole account
- August CTA button A/B — coordinate with DigProf schedule
- Gate: HubSpot phone on Yelp only after team is trained on HubSpot app answering



## Tasks

- Claim / apply $300 promo; confirm it appears on the Ads billing line
- Inventory Yelp upgrades — remove competitor ads, specialty badges, Call to Action variants, etc. — test one change per week
- Set website and post URLs with HubSpot-friendly UTMs — `utm_source=yelp&utm_medium=referral&utm_campaign=pav-yelp`
- Boost Military Criminal Defense Lawyer · Domestic Violence Lawyer · Assault Lawyer · Felony Lawyer · Protection Order / Restraining Order Lawyer as capacity allows
- Keep public 888 / current Yelp call path until HsVoip + LsaCall training gate clears; then point Yelp to HubSpot number for CRM logging
- Block divorce, bankruptcy, PI, theft, free lawyer junk from boost list



## Completed

- Listing / specialties / unique / history / intro / owner copy drafted under CONTENT-WRITING-GUIDE
- DigProf listing refresh and review-ask email in motion — Yelpv1 owns ads performance, promo, upgrades, and tracking



---

—— Unpublished below ——

## Project plan

### Scope

Whole-account Yelp paid + listing performance. Military is a keyword and copy option, not the project identity. Primary criminal voice: rights, record, future · free confidential case evaluation. See CONTENT-WRITING-GUIDE.

### $300 promo

- Apply credit while ads run; do not leave unused past the offer window
- Pair with DigProf August CTA A/B and review volume push
- Track: promo days · spend · leads · calls · cost per lead

### Platform upgrades — performance test

Test one upgrade or package feature at a time for 7 days when possible. Log leads and calls before/after.

Candidates to verify in the live Ads UI:

- Remove competitor ads / competitor page placement controls
- Specialty or category badges
- Call to Action button variants
- Custom ad text vs review snippet vs Yelp auto
- Photo choice
- Keyword boost set changes
- Geo / radius tweaks already in DigProf account

Keep only upgrades that move leads or answered calls. Drop the rest.

### HubSpot links / tracking

- Website field and Connect post Custom URL: pav.law or practice URLs with UTMs
- Default set: `utm_source=yelp&utm_medium=referral&utm_campaign=pav-yelp&utm_content=listing`
- Ad destinations: same campaign name; change `utm_content` only if A/B testing CTAs
- Create HubSpot contacts for real Yelp leads; source Yelp
- Message lead baseline as of 2026-07-29: 3 visible · 2 Criminal defense · 1 Vehicular law · recency 3 / 5 / 13 days · no customer data saved
- At least 1 of 3 visibly shows a reply; response status for the other 2 is not visible in the screenshot
- Do not put phone numbers in Yelp Connect post body if the platform blocks them — use the button URL

### Phone → HubSpot number

- Gate on HsVoip interim HubSpot number + routing and LsaCall / Casey call-answer training
- Until gate clears: keep current public path; never publish HubSpot DID on Yelp early
- After gate: route Yelp calls into HubSpot so activity logs in CRM
- DigProf holds the same gate note — execute the swap once here when training is done

### Custom ad text — primary criminal

Arrested or under investigation? Protect your rights, your record, and your future. Schedule your free and confidential case evaluation with Pav Law today.

### Custom ad text — charge match

Arrested or under investigation? Pav Law defends military, domestic violence, assault, and felony cases in Colorado. Protect your rights, your record, and your future. Free confidential case evaluation. Call today.

### Keywords to boost

Military Criminal Defense Lawyer  
Domestic Violence Lawyer  
Assault Lawyer  
Felony Lawyer  
Protection Order Lawyer  
Restraining Order Lawyer  
Serious Criminal Charges Lawyer  
Weapons Charges Lawyer  

Prefer Lawyer wording. Cap near 12–15 intents. No Attorney twins.

### Do not boost as the lead story

Auto / traffic / DUI · theft · cyber · divorce · bankruptcy · car accident · bare Criminal Defense Lawyer generics

### Geo

Use the affluent / El Paso–weighted set already in the account. Do not rebuild geo for a military-only flight.



## Where to use

- Yelp Ads dashboard — budget, promo, upgrades, custom text, keywords
- Yelp listing — website URL with UTMs, phone after gate
- Yelp Connect posts — headline, description, Custom URL with UTMs
- DigProf — listing refresh, reviews, August CTA calendar
- HsVoip / LsaCall — phone training gate



## Notes

- 2026-07-26: First draft was military-heavy; revised 2026-07-27 to whole-account Yelp performance — promo, upgrades test, HubSpot links, phone gate. Military remains one boost lane.
- 2026-07-29: Updated Yelp message-lead aggregate from inbox screenshot — 3 messages · Criminal defense 2 · Vehicular law 1.
- Related: DigProf · HsVoip · LsaCall · CONTENT-WRITING-GUIDE § Yelp



## Fee note

Market estimate recalculates on `npm run build` from local Front Range + national law-marketing bands × scope. Details: [FEE-BENCHMARK-CO-SPRINGS.md](../FEE-BENCHMARK-CO-SPRINGS.md). Locked Fee / INDEX Est. cost do not change until you copy the market figure in. Market estimate is not shown on Guide cards. Media spend, promo credit, and Yelp upgrade fees are client Yelp budget — not included in the $900 project fee.
